...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    # put segmentation in bbox (revert cropping) if metadata is present; otherwise keep as-is
    if 'bbox_used_for_cropping' in properties_dict and 'shape_before_cropping' in properties_dict:
        ...
        # no crop metadata → skip inserting into original bbox
        if isinstance(segmentation, torch.Tensor):
            segmentation_reverted_cropping = segmentation.cpu().numpy()
        else:
...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    target_file = output_file_truncated + dataset_json_dict_or_file['file_ending']
    print(f"[LOG] Writing segmentation to {target_file}")
    rw.write_seg(segmentation_final, target_file, properties_dict)
